...
def get_card_info_Debit(CardCode=str):
    ...
    # Debit
    Debit_curr1.execute('''SELECT * from DebitInov WHERE CardCode=?''', [CardCode])
...
def charge_card_Debit(ccode=str, name_debit=str, price=float, email_debit=str):
    ...
    # Debit
    charge_debit_curr.execute('''SELECT * from DebitInov WHERE CardCode=?''', [ccode])
    for charge_row in charge_debit_curr.fetchall():
        debit_accnt_address = charge_row[7]
        mail_address = str(debit_accnt_address)
        # charge card
        charge_debit_curr.execute('''UPDATE DebitInov SET Checking=Checking-? WHERE CardCode=?''', [price, ccode])
        conn.commit()
...
def get_card_info_Credit(CardCode=str):
    ...
    # Credit
    Credit_curr1.execute('''SELECT * from CreditInov WHERE CardCode=?''', [CardCode])
...
def charge_card_Credit(ccode=str, name_Credit=str, price=float, email_Credit=str):
    ...
    # Credit
    charge_credit_curr.execute('''SELECT * from CreditInov WHERE CardCode=?''', [ccode])
    for charge_credit_row in charge_credit_curr.fetchall():
        ...
        # charge card
        charge_credit_curr.execute('''UPDATE CreditInov SET Checking=Checking-? WHERE CardCode=?''', [price, ccode])
        conn.commit()
        conn.close()  # completing charge
...
def get_card_info_Crypto(CardCode=str):
    ...
    # Crypto
    Crypto_debit_curr.execute('''SELECT * from CryptoDebitAccounts WHERE CardCode=?''', [CardCode])
...
def charge_card_Crypto(ccode=str, name_crypt=str, price=float, email_crypt=str):
    ...
    # Credit
    charge_crypto_curr.execute('''SELECT * from CryptoDebitAccounts WHERE CardCode=?''', [ccode])
